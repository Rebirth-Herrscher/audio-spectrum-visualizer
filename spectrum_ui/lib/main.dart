...
class SpectrumPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (_style != SpectrumStyle.bars) {
      ...
      if (_style == SpectrumStyle.mirror) {
        final mm = Matrix4.diagonal3Values(1, -1, 1);
        mm.setTranslationRaw(0, size.height, 0);
        canvas.drawPath(
    ...
    for (var i = 0; i < _peaks.length; i += 8) {
      if (_peaks[i] > 0.35 && (DateTime.now().millisecondsSinceEpoch % 2 == 0)) {
        _particles.add(
          _Particle(
            x: i * bw + bw / 2,
            y: size.height - _peaks[i] * size.height,
            vx: (DateTime.now().microsecondsSinceEpoch % 200 - 100) / 150.0,
            speed: 0.8 + _peaks[i] * 2.0,
            life: 1.0,
          ),
        );
      }
    }
    while (_particles.length > 120) {
  ...
  void _drawBarPeaks(Canvas canvas, Size size, double bw) {
    for (var i = 0; i < spectrum.length; i += 4) {
      ...
      canvas.drawCircle(
        Offset(i * bw + bw / 2, size.height - ph),
        1.5,
        Paint()
          ..color = Colors.white.withValues(alpha: _peaks[i].clamp(0.0, 0.9))
          ..maskFilter = const MaskFilter.blur(BlurStyle.normal, 2),
      );
    }
  }
  ...
  @override
  bool shouldRepaint(covariant SpectrumPainter o) => true;
}
...
class _BgPainter extends CustomPainter {
  ...
  @override
  bool shouldRepaint(covariant _BgPainter o) => o.theme != theme;
}
