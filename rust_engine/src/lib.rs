...
#[allow(non_snake_case)] unsafe extern "system" { fn CoInitializeEx(r: *mut std::ffi::c_void, c: u32) -> i32; fn CoUninitialize(); }
const COINIT_MULTITHREADED: u32 = 0x0;

#[repr(C)] pub struct KissFftCpx { pub r: f32, pub i: f32 }
impl Clone for KissFftCpx { fn clone(&self) -> Self { Self { r: self.r, i: self.i } } }
#[repr(C)] pub struct WasapiConfig { pub sample_rate: u32, pub buffer_frames: u32, pub channels: u32 }
...
pub extern "C" fn engine_start_capture(sr: u32, ch: u32, bf: u32, nfft: u32, sp: u32) -> i32 {
    if RUNNING.load(Ordering::Acquire) || !THREAD_DONE.load(Ordering::Acquire) { return -1; }
    SPECTRUM_LEN.store(sp, Ordering::Release);
    THREAD_DONE.store(false, Ordering::Release);
    RUNNING.store(true, Ordering::Release);
    let config = WasapiConfig { sample_rate: sr, buffer_frames: bf, channels: ch };
    let _ = thread::Builder::new().stack_size(2*1024*1024).spawn(move || {
        unsafe { CoInitializeEx(std::ptr::null_mut(), COINIT_MULTITHREADED); }
        let mut ctx: *mut std::ffi::c_void = std::ptr::null_mut();
        unsafe { if wasapi_create(&mut ctx, &config) != 0 || wasapi_get_default_device(ctx) != 0 || wasapi_init_client(ctx, true) != 0 || wasapi_start(ctx) != 0
            { if !ctx.is_null() { wasapi_destroy(ctx); } CoUninitialize(); RUNNING.store(false, Ordering::Release); THREAD_DONE.store(true, Ordering::Release); return; } }
        ...
        if fft_plan.is_null() || log_map.is_null() {
            if !fft_plan.is_null() { unsafe { fft_destroy(fft_plan); } }
            if !log_map.is_null() { unsafe { log_map_destroy(log_map); } }
            unsafe { wasapi_stop(ctx); wasapi_destroy(ctx); CoUninitialize(); }
            RUNNING.store(false, Ordering::Release); THREAD_DONE.store(true, Ordering::Release); return;
        }
        let nb = (nfft/2+1) as usize;
        let win: Vec<f32> = (0..nfft as usize).map(|i| 0.5*(1.0-(2.0*std::f32::consts::PI*i as f32/(nfft-1) as f32).cos())).collect();
        let mut ring = vec![0.0f32; nfft as usize]; let mut rp = 0usize;
        ...
        while RUNNING.load(Ordering::Acquire) {
            ...
            if read > 0 { for f in 0..read as usize {
                ...
                let mut s = 0.0f32; for c in 0..ch as usize { s += buf[f*ch as usize+c]; }
                ring[rp] = s / ch as f32; rp += 1;
            }} else {
                let n = rf as usize; if rp+n >= nfft as usize { rp = 0; }
                ring[rp..rp+n].fill(0.0); rp += n;
            }
            // Run FFT with sliding window on every iteration
            {
                ...
                unsafe { fft_execute(fft_plan, fin.as_ptr(), fout.as_mut_ptr()); fft_magnitude(fout.as_ptr(), mag.as_mut_ptr(), nfft); }
                mag[0] = 0.0;
                unsafe { log_map_apply(mag.as_ptr(), log_map, spec.as_mut_ptr(), sp); }
...
#[unsafe(no_mangle)] pub extern "C" fn engine_stop_capture() {
    RUNNING.store(false, Ordering::Release);
    for _ in 0..100 { if THREAD_DONE.load(Ordering::Acquire) { break; } thread::sleep(std::time::Duration::from_millis(10)); }
}

#[unsafe(no_mangle)] pub extern "C" fn engine_read_spectrum(buffer: *mut f32, len: i32) -> i32 {
